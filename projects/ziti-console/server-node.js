...
app.use('/', express.static(__dirname + '/../../dist/app-open-ziti-node'));
app.use('/:name', express.static(__dirname + '/../../dist/app-open-ziti-node'));
...
app.get('/:name', function(request, response) {
	...
	console.log("Resource requested: " + name);
	console.log("Should send resource: " + __dirname + '/../../dist/app-open-ziti-node/'+name);
	...
	if(isResource) {
		console.log("Resource sent: " + __dirname + '/../../dist/app-open-ziti-node/'+name);
		response.sendFile(path.resolve(__dirname + '/../../dist/app-open-ziti-node/'+name));
	} else {
		response.sendFile(path.join(__dirname + '/../../dist/app-open-ziti-node/index.html'));
	}
});

app.get('/assets/pages/:name', function(request, response) {
	var name = request.params.name;
	console.log("Page requested: " + name);
	console.log("Page sent: " + __dirname + '/../../dist/app-open-ziti-node/assets/pages/'+name);
	response.sendFile(path.resolve(__dirname + '/../../dist/app-open-ziti-node/assets/pages/'+name));
});

app.get('/:resource/:name', function(request, response) {
	var name = request.params.name;
	var resource = request.params.resource;
	console("Resource requested: " + resource + '/' + name);
	console("Resource sent: " + __dirname + '/../../dist/app-open-ziti-node/'+resource+'/'+name);
	response.sendFile(path.resolve(__dirname + '/../../dist/app-open-ziti-node/'+resource+'/'+name));
});
*/
/**
	...
	if (bindIP=="" || bindIP==null) {
		if (zitified) {
			...
			server = app.listen(startupPort, function() {
				console.log("Ziti Admin Console is now listening on port "+startupPort+ " via localhost");
		...
		if (zitified) {
			...
			server = app.listen(startupPort, bindIP, function() {
				console.log("Ziti Admin Console is now listening on port "+startupPort+ " via IP: " + bindIP);
			}).on('error', function(err) {
				if (err.code=="EADDRINUSE") {
					maxAttempts--;
					console.log("Port "+startupPort+" In Use, Attempting new port "+(startupPort+1));
